...
__device__ double distance_2d(double self_rx, double self_ry, double neigh_rx, double neigh_ry){//distance between 2 particles
...
*/
__device__ double distance_2d_periodic(double self_rx, double self_ry, double neigh_rx, double neigh_ry, double xbound, double ybound){
...
__device__ double f(double q){//f(q) in equation (B2) in https://academic.oup.com/mnras/article/401/3/1475/1089493 
...
__device__ double fab(double h, double distance, int d){ 
...
__device__ double p(double density, double u){
...
__device__ double cal_cs(double u){//u: internal energy of the particle
...
__device__ double coor_difference(double coor_a, double coor_b, double bound){
...
__device__ double vabrab(double vax, double vay, double vbx, double vby, double xa, double ya, double xb, double yb, double xbound, double ybound){
...
__device__ double v_sig(double ua, double ub, double vax, double vay, double vbx, double vby, double xa, double ya, double xb, double yb, double xbound, double ybound){
...
__device__ double cubic_spline_kernel(double self_rx, double self_ry, double neigh_rx, double neigh_ry, double h, double d){ 
    ...
    return (norm / pow(h, d)) * ((1 - (3.0 / 2.0) * q * q + (3.0 / 4.0) * q * q * q) * (0 <= q) * (q < 1) + (1.0 / 4.0) * pow(2 - q, 3) * (1 <= q) * (q < 2));
...
__device__ double first_derivative_kernel(double self_rx, double self_ry, double neigh_rx, double neigh_ry, double h, double d){ 
    
    double distance = distance_2d(self_rx, self_ry, neigh_rx, neigh_ry); //distance between two particles
    double q = distance / h; // variable q in the math formulation
    double norm = (2.0 / 3.0) * (d == 1) + (10.0 / (7.0 * M_PI)) * (d == 2) + (1.0 / M_PI) * (d == 3); // σ normalisation factor

    return (-norm / pow(h, d + 1)) * (d * ((1 - (3.0 / 2.0) * q * q + (3.0 / 4.0) * q * q * q) * (0 <= q) * (q < 1) + (1.0 / 4.0) * pow(2 - q, 3) * (1 <= q) * (q < 2)) + 
    q * ((q * (9.0 * q / 4.0 - 3.0)) * (0 <= q) * (q < 1) + (-3.0 * (q - 2) * (q - 2) / 4.0) * (1 <= q) * (q < 2)));
...
__device__ double cubic_spline_kernel_periodic(double self_rx, double self_ry, double neigh_rx, double neigh_ry, double h, double d, double xbound, double ybound){ 
...
__device__ double first_derivative_kernel_periodic(double self_rx, double self_ry, double neigh_rx, double neigh_ry, double h, double d, double xbound, double ybound){ 
